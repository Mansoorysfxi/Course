...
from app.routers import agent, auth, notes, quests
...
# NEW in Module 14 -- see app/routers/notes.py. Registered after
# `quests.router` for readability only; FastAPI resolves routes by their
# own path template, not by registration order between separate routers
# with non-overlapping prefixes, so this ordering has no runtime effect.
app.include_router(notes.router)
# NEW in Module 15 -- see app/routers/agent.py. Same "registration order
# doesn't matter, non-overlapping prefixes" note as notes.router above.
