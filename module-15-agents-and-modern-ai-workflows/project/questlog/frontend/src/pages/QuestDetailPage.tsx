...
import { QuestBreakdownPanel } from "../components/QuestBreakdownPanel";
import { QuestNotesPanel } from "../components/QuestNotesPanel";
...
export function QuestDetailPage() {
  ...
  const { getQuest, updateQuest, deleteQuest, toggleDone, addQuest } = useQuests();
  ...
  // NEW in Module 13 -- accepting an AI-suggested sub-quest is just
  // `addQuest` (QuestsContext, unchanged since Module 05) with a
  // `NewQuestInput` this page builds itself. QuestBreakdownPanel knows
  // nothing about QuestsContext at all -- it only calls whatever function
  // its `onAcceptSuggestion` prop was given, the same "the component
  // collects input, its parent decides what to do with it" pattern
  // QuestForm's `onSubmit` prop already established. The sub-quest
  // inherits its parent's priority and quest line so it shows up
  // alongside the quest it was broken down from on the Quest Board.
  ...
  return (
    <div className="mx-auto max-w-lg">
      ...
      {/* NEW in Module 14 -- "chat with your quest notes." A sibling panel
       * to QuestBreakdownPanel above, not a replacement -- this page now
       * hosts two separate AI features, each scoped to its own component,
       * the same "the page composes, components own their own state" shape
       * this file has followed since Module 04. */}
      <QuestNotesPanel questId={quest.id} />
    </div>
  );
}
