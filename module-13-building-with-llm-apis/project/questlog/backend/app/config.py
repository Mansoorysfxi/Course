...
class Settings(BaseSettings):
    ...
    # NEW in Module 10 -- where the Redis cache (app/cache.py) connects.
    # `redis://localhost:6379/0` matches Redis's own connection-URL scheme
    # (redis://<host>:<port>/<database-number>; Redis ships 16 numbered
    # databases per instance by default, 0 through 15, and QuestLog uses
    # just the default, 0) and is correct for local development, whether
    # you run Redis directly or via this module's own `docker compose`
    # setup (see lessons/06-docker-compose-multi-service-apps.md and
    # lessons/08-containerizing-questlogs-frontend-and-full-compose.md).
    # Inside Docker Compose, this same field is instead set to
    # `redis://redis:6379/0` via an environment variable in
    # `docker-compose.yml` -- `redis` there is not a placeholder, it is a
    # real, resolvable hostname, because Compose gives every service a
    # DNS name equal to its own service name on the network it creates.
    # See lessons/04-docker-networking.md for exactly why that works.
    redis_url: str = "redis://localhost:6379/0"

    # NEW in Module 11 -- see app/main.py and
    # lessons/06-monitoring-logging-and-error-tracking.md for the full
    # explanation. `sentry_dsn` has a default of `None` (unlike
    # `secret_key` above) because error tracking is genuinely optional --
    # this app must run correctly with no Sentry account at all (local
    # dev, CI, and every exercise in this module never set it); Sentry is
    # only initialized in app/main.py if a real value is present. `None`,
    # not an empty string, is the "off" value, because pydantic-settings
    # itself would otherwise happily accept the *empty string* an unset
    # environment variable sometimes resolves to, which `sentry_sdk.init`
    # would then try to use as a real (invalid) DSN.
    sentry_dsn: str | None = None

    # A short label Sentry attaches to every error/event it receives, so a
    # real Sentry dashboard can separate "this happened in production" from
    # "this happened while a learner was running the test suite locally" --
    # see lessons/06's own section on why `environment` is one of the very
    # first things any error-tracking setup should configure.
    environment: str = "development"

    # NEW in Module 13 -- see app/ai_assistant.py and
    # lessons/07-building-questlogs-ai-assistant-backend.md. Exactly the
    # same "optional, `None` is the off value" shape as `sentry_dsn` above,
    # for exactly the same reason: this app must run correctly, and its
    # entire test suite must pass, with no Anthropic API key configured at
    # all (see tests/test_ai_assistant.py). `app/dependencies.py`'s
    # `get_ai_client` is the one place this field is ever read.
    anthropic_api_key: str | None = None

    # Which Claude model powers the quest-breakdown feature. A field, not a
    # hardcoded string inside app/ai_assistant.py, for the same reason
    # every other real setting in this class is a field: swapping models
    # (e.g. to test a more capable one) should never require editing code.
    # `claude-haiku-4-5` is a **fixed technology decision** for this
    # feature specifically -- see RUNNING_PROJECT.md's "Fixed technology
    # decisions" section and lessons/00-setup.md's cost table for why.
