...
(Module 06) for `DbSession`, unchanged here, and
lessons/07-protecting-routes-with-dependencies.md (this module) for
everything new below: `oauth2_scheme`, `get_current_user`, `CurrentUser`,
and the now-authenticated `get_quest_or_404`.
"""
...
async def get_quest_or_404(
    quest_id: str, session: DbSession, current_user: CurrentUser
) -> Quest:
