...
class FakeRedis:
    ...
    Implements the four methods this codebase actually calls on a Redis
    client (`get`, `set`, `delete` from app/routers/quests.py; `ping`,
    NEW in Module 11, from app/main.py's `/health` endpoint) with the
    exact same async signatures `redis.asyncio.Redis` has -- enough for
    `app.dependency_overrides` to swap this in wherever a route asks for
    `RedisClient`, with the route code itself never able to tell the
    difference. `ex=` (the TTL, in seconds) is accepted for
    signature-compatibility but not actually enforced here -- no test in
    this suite runs long enough for a 30-second TTL to matter; what these
    tests actually verify is that a write correctly calls `delete` at
    all, not real time-based expiry.
    """
    ...
    async def ping(self) -> bool:
        # A real `redis.asyncio.Redis.ping()` sends Redis's own `PING`
        # command and returns `True` once the server replies `PONG` --
        # this fake has no real server to reach, so it simply always
        # succeeds, mirroring "Redis is up" being the normal case this
        # test suite runs against. tests/test_health.py has no test for
        # the *unreachable* branch of app/main.py's `/health` for exactly
        # this reason: simulating a genuinely broken connection isn't
        # something a fake this simple can honestly do -- see that
        # lesson's own "what this course's tests don't (and can't
        # honestly) cover" note.
        return True
