# QuestLog API — backend (Module 10: containerized, gains a Redis-backed cache)
...
carried forward through Module 08 (which added a real, passing pytest
test suite — `tests/` — 31 tests: `test_security.py`, `test_auth.py`,
`test_quests.py` — plus `ruff` for linting/formatting), Module 09
(deployed manually to a real server — no code changes at all), and now
Module 10, which adds exactly two things: a `Dockerfile` (see that file,
and [`lessons/07-containerizing-questlogs-backend.md`](../../../lessons/07-containerizing-questlogs-backend.md))
and a small, real, documented Redis cache for `GET /api/quests`'s
unfiltered quest list (`app/cache.py`, new; `app/config.py`,
`app/dependencies.py`, and `app/routers/quests.py` each touched
minimally — see [`../README.md`](../README.md)'s "What changed in
`backend/app/`" section for the exact, complete list, and
[`tests/test_caching.py`](tests/test_caching.py) — 6 new tests, 37 total,
all passing). `app/repository.py` and every other file are untouched.
...
For this module's own deployment material, see
[`../../../lessons/`](../../../lessons/), and
...
python -m pytest -v          # expect: 37 passed (31 unchanged + 6 new caching tests)
...
| **redis** | **8.1.0** | NEW in Module 10 — `https://pypi.org/project/redis/`, verified August 2026; provides `redis.asyncio`, this app's async client. The old standalone `aioredis` package is archived/deprecated (merged into `redis-py` since 4.2.0) and not used here. |
...
├── config.py           — pydantic-settings Settings (DATABASE_URL, SECRET_KEY, ALGORITHM, ACCESS_TOKEN_EXPIRE_MINUTES, CORS_ORIGINS, NEW: REDIS_URL)
...
├── cache.py                  — NEW in Module 10: Redis client (redis.asyncio), quest-list cache-key + TTL helpers
...
├── repository.py                   — every real database query, scoped by owner_id — UNTOUCHED by Module 10's caching
├── dependencies.py                   — DbSession, oauth2_scheme, get_current_user, CurrentUser, get_quest_or_404, NEW: RedisClient
└── routers/
    ├── auth.py                          — POST /api/auth/signup, POST /api/auth/login, GET /api/auth/me
    └── quests.py                         — every /api/quests route; NEW in Module 10: list_quests reads/writes the Redis cache, create/update/delete invalidate it
alembic/
└── versions/
...
Dockerfile               — NEW in Module 10: multi-stage Python build, see lessons/07
.dockerignore              — NEW in Module 10
.env.example            — template for the required .env (never commit the real .env)
requirements.txt
```
