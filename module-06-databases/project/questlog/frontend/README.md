# QuestLog (web) — frontend, unchanged in Module 06

This is **Module 05's finished `questlog` frontend, copied forward
unmodified** (per `RUNNING_PROJECT.md`'s repo-location convention). Module
06's entire job happened in `../backend/` — swapping its in-memory `dict`
for real PostgreSQL persistence — and per
[`../../BRIEF.md`](../../BRIEF.md)'s "the contract doesn't change" section,
that swap was deliberately invisible to this frontend: same routes, same
JSON shapes, same errors. Not one file in this folder was touched in this
module. See [`../README.md`](../README.md) for exactly how this `frontend/`
folder relates to `../backend/`.
...
persisted to, a real, separately-running FastAPI process — as of this
module, one backed by a real PostgreSQL database instead of an in-memory
dict, though this frontend has no way to tell the difference and needed no
changes to keep working.

## What changed from the Module 05 version

Nothing. This section is intentionally short: per this module's own
`README.md`, the entire point of Module 06 was proving that a real
database swap-in requires zero frontend changes when the API contract is
held fixed. For the last time any of this frontend's code *did* change
(Module 04 → Module 05, adding the real backend in the first place), see
the version of this file included in
[`module-05-backend-fastapi/project/questlog/frontend/README.md`](../../../../module-05-backend-fastapi/project/questlog/frontend/README.md).

For reference, here is what changed back in Module 05, which still stands:
