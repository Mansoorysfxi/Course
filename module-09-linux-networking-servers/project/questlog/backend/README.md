...
`test_quests.py` — plus `ruff` for linting/formatting) and now Module 09
(**application code and tests here are byte-for-byte unchanged from
Module 08** — this module only adds deployment artifacts, in
[`../deploy/`](../deploy/), never touching this folder's own code). For
the testing material itself, taught in Module 08, see
